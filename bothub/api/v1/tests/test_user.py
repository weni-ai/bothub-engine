...
class RegisterUserTestCase(TestCase):
    ...
    def request(self, data):
        request = self.factory.post("/v1/register/", data)
...
class LoginTestCase(TestCase):
    ...
    def request(self, data):
        request = self.factory.post("/v1/login/", data)
...
class ChangePasswordTestCase(TestCase):
    ...
    def request(self, data, token):
        authorization_header = {"HTTP_AUTHORIZATION": "Token {}".format(token.key)}
        request = self.factory.post("/v1/", data, **authorization_header)
...
class RequestResetPasswordTestCase(TestCase):
    ...
    def request(self, data):
        request = self.factory.post("/v1/forgot-password/", data)
        response = RequestResetPassword.as_view({"post": "create"})(request)
        response.render()
        content_data = json.loads(response.content)
        return (response, content_data)
...
class ResetPasswordTestCase(TestCase):
    ...
    def request(self, nickname, data):
        request = self.factory.post("/v1/reset-password/{}/".format(nickname), data)
        response = ResetPassword.as_view({"post": "update"})(request, nickname=nickname)
        response.render()
        content_data = json.loads(response.content)
        return (response, content_data)
...
class MyUserProfileTestCase(TestCase):
    ...
    def request(self, token):
        authorization_header = {"HTTP_AUTHORIZATION": "Token {}".format(token.key)}
        request = self.factory.get("/v1/my-profile/", **authorization_header)
