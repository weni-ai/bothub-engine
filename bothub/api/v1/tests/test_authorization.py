...
class AuthorizationTestCase(TestCase):
    ...
    def request(self, repository, token):
        ...
        response = RepositoryViewSet.as_view({"get": "authorization"})(
    ...
    def test_owner_allowed_public(self):
        ...
        self.assertEqual(content_data.get("user"), self.owner.id)

    def test_user_allowed_public(self):
        response, content_data = self.request(self.repository, self.user_token)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(content_data.get("user"), self.user.id)
...
class UpdateAuthorizationRoleTestCase(TestCase):
    ...
    def request(self, repository, token, user, data):
        ...
        request = self.factory.patch(
            "/v1/authorization-role/{}/{}/".format(repository.uuid, user.nickname),
            self.factory._encode_data(data, MULTIPART_CONTENT),
            MULTIPART_CONTENT,
            **authorization_header
        )
        view = RepositoryAuthorizationRoleViewSet.as_view({"patch": "update"})
        response = view(
