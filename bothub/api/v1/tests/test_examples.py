...
class ExamplesTestCase(TestCase):
    def setUp(self):
        ...
        self.translated = RepositoryTranslatedExample.objects.create(
            original_example=self.example, text="oi", language=languages.LANGUAGE_PT
        )
        self.deleted = RepositoryExample.objects.create(
            repository_update=self.repository.current_update(), text="hey"
        )
        self.deleted.delete()
    ...
    def request(self, data, token):
        ...
        request = self.factory.get("/v1/examples/", data, **authorization_header)
        response = RepositoryExamplesViewSet.as_view({"get": "list"})(request)
        response.render()
    ...
    def test_language_filter(self):
        ...
        self.assertGreaterEqual(content_data_1.get("count"), 1)
    ...
    def test_has_translation_filter(self):
        response_1, content_data_1 = self.request(
            {"repository_uuid": self.repository.uuid, "has_translation": True},
            self.owner_token,
        )
        self.assertEqual(response_1.status_code, status.HTTP_200_OK)
        self.assertGreaterEqual(content_data_1.get("count"), 1)
        ...
        response_2, content_data_2 = self.request(
            {"repository_uuid": self.repository.uuid, "has_translation": False},
            self.owner_token,
        )
        self.assertEqual(response_2.status_code, status.HTTP_200_OK)
        self.assertEqual(content_data_2.get("count"), 0)
    ...
    def test_forbidden(self):
        ...
        response, content_data = self.request(
            {"repository_uuid": self.private_repository.uuid}, user_token
        )
    ...
    def test_order_by_translation(self):
        RepositoryExample.objects.create(
            repository_update=self.repository.current_update(), text="hello"
        )
        example = RepositoryExample.objects.create(
            repository_update=self.repository.current_update(), text="how are you?"
        )
        ...
        self.assertGreaterEqual(0, len(results[0].get("translations")))
        self.assertGreaterEqual(0, len(results[1].get("translations")))
        self.assertGreaterEqual(1, len(results[2].get("translations")))
        self.assertGreaterEqual(1, len(results[3].get("translations")))
    ...
    def test_order_by_translation_inverted(self):
        ...
        results = content_data.get("results")
        self.assertGreaterEqual(1, len(results[0].get("translations")))
        self.assertGreaterEqual(1, len(results[1].get("translations")))
        self.assertGreaterEqual(0, len(results[2].get("translations")))
        self.assertGreaterEqual(0, len(results[3].get("translations")))
    ...
    def test_has_not_translation_to(self):
        ...
        self.assertEqual(content_data.get("results")[1].get("id"), example_1.id)
        self.assertEqual(content_data.get("results")[0].get("id"), example_2.id)
