...
class NewRepositoryTestCase(TestCase):
    ...
    def request(self, data):
        request = self.factory.post(
            "/v1/repository/new/", data, **self.authorization_header
        )
    ...
    def test_fields_required(self):
        ...
        request_and_check(
            "language",
            {"name": "Testing", "slug": "test", "categories": [self.category.id]},
        )

        request_and_check(
            "categories",
            {"name": "Testing", "slug": "test", "language": languages.LANGUAGE_EN},
        )
...
class RetrieveRepositoryTestCase(TestCase):
    ...
    def request(self, repository, token):
        ...
        request = self.factory.get(
            "/v1/repository/{}/{}/".format(repository.owner.nickname, repository.slug),
            **authorization_header
        )
        response = RepositoryViewSet.as_view({"get": "retrieve"})(
            request, owner__nickname=repository.owner.nickname, slug=repository.slug
        )
        response.render()
        content_data = json.loads(response.content)
        return (response, content_data)
    ...
    def test_languages_status(self):
        ...
        request = self.factory.get(
            "/v1/repository/{}/{}/languagesstatus/".format(
                self.repository.owner.nickname, self.repository.uuid
            ),
            **authorization_header
        )
...
class UpdateRepositoryTestCase(TestCase):
    ...
    def test_allowed(self):
        new_slug = "test_2"
        response, content_data = self.request(
            self.repository, self.owner_token, {"slug": new_slug}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_forbidden(self):
        new_slug = "test_2"
        response, content_data = self.request(
            self.repository, self.user_token, {"slug": new_slug}
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_update_fields(self):
        new_category = RepositoryCategory.objects.create(name="Other")
        mockups = [
            ("name", "New Name", True),
            ("language", languages.LANGUAGE_PT, True),
            ("categories", [self.category.id, new_category.id], True),
            ("description", "Add description", True),
            ("is_private", True, True),
            ("uuid", uuid.uuid4(), False),
            ("slug", "test-slug", True),
        ]
        for (field, value, equal) in mockups:
            response, content_data = self.request(
                self.repository, self.owner_token, {field: value}
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            if equal:
                self.assertEqual(content_data.get(field), value)
            else:
                self.assertNotEqual(content_data.get(field), value)
...
class DestroyRepositoryTestCase(TestCase):
    ...
    def request(self, repository, token):
        ...
        response = RepositoryViewSet.as_view({"delete": "destroy"})(
            request, owner__nickname=repository.owner.nickname, slug=repository.slug
        )
        response.render()
        return response
    ...
    def test_allowed(self):
        response = self.request(self.repository, self.owner_token)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_forbidden(self):
        response = self.request(self.repository, self.user_token)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class RepositoriesTestCase(TestCase):
    ...
    def request(self, data={}):
        request = self.factory.get("/v1/repositories/", data)
    ...
    def test_filter_by_name(self):
        response, content_data = self.request({"search": "Test"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(content_data.get("count"), 1)

        response, content_data = self.request({"search": "z"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(content_data.get("count"), 0)
    ...
    def request(self, repository, token):
        ...
        request = self.factory.get(
            "/v1/repository/{}/{}/train/".format(
                repository.owner.nickname, repository.slug
            ),
            **authorization_header
        )
...
class AnalyzeRepositoryTestCase(TestCase):
    ...
    def test_permission_denied_in_private_repository(self):
        response, content_data = self.request(
            ...
            self.user_token,
            {"language": "en", "text": "My name is user"},
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class LanguagesStatusTestCase(TestCase):
    ...
    def test_unique_entities(self):
        RepositoryExampleEntity.objects.create(
            repository_example=self.example_1, start=0, end=0, entity="entity1"
        )
        RepositoryExampleEntity.objects.create(
            repository_example=self.example_2, start=0, end=0, entity="entity1"
        )
        RepositoryExampleEntity.objects.create(
            repository_example=self.example_3, start=0, end=0, entity="entity2"
        )
