...
class TranslateExampleTestCase(TestCase):
    ...
    def request(self, data, user_token):
        ...
        response = NewRepositoryTranslatedExampleViewSet.as_view({"post": "create"})(
            request
        )
        response.render()
...
class RepositoryTranslatedExampleRetrieveTestCase(TestCase):
    ...
    def request(self, translated, token):
        ...
        request = self.factory.get(
            "/v1/translation/{}/".format(translated.id), **authorization_header
        )
...
class TranslationsViewTest(TestCase):
    ...
    def request(self, data, user_token=None):
        ...
        request = self.factory.get("/v1/translations/", data, **authorization_header)
    ...
    def test_filter_from_language(self):
        example = RepositoryExample.objects.create(
            repository_update=self.repository.current_update(languages.LANGUAGE_ES),
            text="hola",
        )
